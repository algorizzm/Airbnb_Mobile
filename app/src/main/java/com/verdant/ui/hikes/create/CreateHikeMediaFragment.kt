...
class CreateHikeMediaFragment : Fragment(R.layout.fragment_create_hike_media) {
    ...
    private val galleryPicker = registerForActivityResult(
        ...
        val idx = pendingGalleryIndex ?: return@registerForActivityResult
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        _binding = FragmentCreateHikeMediaBinding.bind(view)

        binding.btnPickCover.setOnClickListener {
            coverPicker.launch("image/*")
        }

        binding.recyclerGallery.layoutManager = LinearLayoutManager(requireContext())
        val adapter = GallerySlotAdapter(
    ...
    private class GallerySlotAdapter(
        ...
        private var urls: List<String> = List(slotCount) { "" }

        fun submit(list: List<String>) {
            val m = list.toMutableList()
            while (m.size < slotCount) m.add("")
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
            ...
            val b = ItemCreateHikeMediaSlotBinding.inflate(inf, parent, false)
            return VH(b, onPick)
        }

        override fun onBindViewHolder(holder: VH, position: Int) {
            holder.bind(position, urls.getOrElse(position) { "" })
        }
        ...
        class VH(
            ...
            fun bind(index: Int, url: String) {
                binding.tvSlotLabel.text = "Gallery item ${index + 1}"
                binding.btnPick.setOnClickListener { onPick(index) }
                if (url.isNotBlank()) {
                    Glide.with(binding.imgPreview).load(url).centerCrop().into(binding.imgPreview)
                } else {
                    Glide.with(binding.imgPreview).clear(binding.imgPreview)
                }
            }
        }
    }
